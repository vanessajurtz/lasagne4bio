...
"""
LSTM net training using the Lasagne library:
...
parser.add_argument('-n_lstm', '--n_lstm',  help="Number of LSTM units, default = 20", default=20)
...
parser.add_argument('-architecture', '--architecture',  help="Neural network architecture, default = lstm", default="lstm")
...
try:
    ...
    print("# number LSTM memory blocks: " + str(N_LSTM))
except:
    sys.stderr.write("Problem with number of LSTM units specification (option -n_lstm)!\n")
    sys.exit(2)
...
N_FEATURES = X_pep_train[0].shape[1]
...
X_train,X_train_mask = data_io_func.pad_pep_mhc_mask_final(X_pep_train, X_mhc_train, MAX_PEP_SEQ_LEN, MHC_SEQ_LEN)
X_val,X_val_mask = data_io_func.pad_pep_mhc_mask_final(X_pep_val, X_mhc_val, MAX_PEP_SEQ_LEN, MHC_SEQ_LEN)
...
if ARCHITECTURE == "lstm":
    ...
    net_hyper_params=np.array([N_FEATURES, N_LSTM, ACTIVATION, DROPOUT, N_HID, W_INIT])
else:
...
train_fn = theano.function([in_pep_mhc.input_var, in_pep_mhc_mask.input_var, sym_target, sym_l_rate], loss, updates=updates, on_unused_input='warn')
...
if NO_EARLY_STOP == False:
    ...
    val_fn = theano.function([in_pep_mhc.input_var, in_pep_mhc_mask.input_var, sym_target], test_loss, on_unused_input='warn')
...
for e in EPOCHS:
    ...
    for batch in iterate_minibatches(X_train, X_train_mask, y_train, BATCH_SIZE):
        pep_mhc, pep_mhc_mask, targets = batch
        train_err += train_fn(pep_mhc, pep_mhc_mask, targets.flatten(), LEARNING_RATE)
        train_batches += 1

    # predict validation set:
    if NO_EARLY_STOP == False:
        for batch in iterate_minibatches(X_val, X_val_mask, y_val, BATCH_SIZE):
            pep_mhc, pep_mhc_mask, targets = batch
            val_err += val_fn(pep_mhc, pep_mhc_mask, targets.flatten())
            val_batches += 1
...
bn_update_fn = theano.function([in_pep_mhc.input_var, in_pep_mhc_mask.input_var, sym_target], loss, on_unused_input='warn')


# predict training set:
for batch in iterate_minibatches(X_train, X_train_mask, y_train, len(X_train)):
    pep_mhc, pep_mhc_mask, targets = batch
    tmp = bn_update_fn(pep_mhc, pep_mhc_mask, targets.flatten())
